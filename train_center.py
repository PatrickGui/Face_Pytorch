...
from utils.config import DataPrefetcher, setup_seed
...
from lossfunctions.centerloss import CenterLoss, CenterLoss_back, FisherLoss
from utils.logging_ import init_log
...
def train(args):
    ...
    criterion_center = FisherLoss(trainset.class_nums, args.feature_dim).to(device)
    # criterion_center = loss.CenterLoss(trainset.class_nums, args.feature_dim).to(device)
    ...
    for epoch in range(1, args.total_epoch + 1):
        ...
        while batch is not None:
            ...
            if total_iters % 200 == 0:
                ...
                total = label.size(0)
                # correct = (np.array(predict.cpu()) == np.array(label.data.cpu())).mean()
                accuracy = (label.data == predict).float().mean()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_root', type=str, default='/raid_new/FZQ/data/CASIA-webface-112x112', help='train image root')
    parser.add_argument('--train_file_list', type=str, default='/raid_new/GPH/Face_Pytorch-master/dataset/CASIA-WebFace-112X112.txt', help='train list')
    parser.add_argument('--lfw_test_root', type=str, default='/raid_new/GPH/DataFace/LFW/lfw_align_112', help='lfw image root')
    parser.add_argument('--lfw_file_list', type=str, default='/raid_new/GPH/DataFace/LFW/pairs.txt', help='lfw pair file list')
    parser.add_argument('--agedb_test_root', type=str, default='/raid_new/GPH/DataFace/AgeDB-30/agedb30_align_112', help='agedb image root')
    parser.add_argument('--agedb_file_list', type=str, default='/raid_new/GPH/DataFace/AgeDB-30/agedb_30_pair.txt', help='agedb pair file list')
    parser.add_argument('--cfpfp_test_root', type=str, default='/raid_new/GPH/DataFace/CFP-FP/CFP_FP_aligned_112', help='agedb image root')
    parser.add_argument('--cfpfp_file_list', type=str, default='/raid_new/GPH/DataFace/CFP-FP/cfp_fp_pair.txt', help='agedb pair file list')

    parser.add_argument('--backbone', type=str, default='Res50_IR', help='MobileFace, Res50, Res101, Res50_IR, SERes50_IR, SphereNet')
    parser.add_argument('--margin_type', type=str, default='ArcFace', help='InnerProduct, ArcFace, CosFace, SphereFace')
    ...
    parser.add_argument('--model_pre', type=str, default='128_center_fisher_w_0_01_', help='model prefix')
    parser.add_argument('--gpus', type=str, default='3,1,2,0', help='model prefix')
